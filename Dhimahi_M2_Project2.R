...
p_load(FSA,FSAdata,magrittr,dplyr,plotrix,ggplot2,moments)
...
plot(age ~ fl, data = sampel1, xlim=c(0,500), ylim=c(0,15), las=2,
     ylab = "Age (yrs)", xlab ="Fork Length (mm)", main="Plot 1: Harrison Lake Trout")

#an "Age" histogram 
hist(age, xlab = "Age (yrs)", main = "Plot 2: Harrison Fish Age Distribution", 
     xlim =c(0,15), ylim =c(0,15), las=2, col = "cadetblue")

#an overdense plot
plot(age ~ fl, data = sampel1, xlim=c(0,500), ylim=c(0,15), las= 2, pch=16, col="green",
...
plot(age ~ fl, data = sampel1, xlim=c(0,500), ylim=c(0,15), las= 2, pch=pchs[sample1$era], col=cols[sample1$era],
